...
| AC7 | Notification failure independent from workflow state | Pass | Workflow commits precede notification recording in the application service; the unavailable adapter records a failed attempt with an independent bounded token in `src/VolunteerCoordinator.Infrastructure/Notifications/UnavailableNotificationService.cs:20-33`; browser transitions completed with the notification-unavailable warning, and PostgreSQL contained 4 failed attempts out of 4 total attempts after the exercised flow. |
